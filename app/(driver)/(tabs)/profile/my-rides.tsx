...
export default function PublishedRides() {
  ...
  const activeRides = useMemo(
    () =>
      rides.filter(
        (ride) => ride.status === "pending" || ride.status === "ongoing"
      ),
    [rides]
  );
  ...
  return (
    <View className="flex-1 bg-gray-50">
      {/* Header */}
      <View className="bg-white pt-6 pb-4 px-4 border-b border-gray-200">
        <Text className="text-gray-900 font-poppinsBold text-2xl">
          {t("driver.tabs.myTrip.publishedRides.header")}
        </Text>
        <Text className="text-gray-600 font-poppins text-sm mt-1">
          {activeRides.length}{" "}
          {t("driver.tabs.myTrip.publishedRides.activeRides")}
          {activeRides.length !== 1 ? "s" : ""}
          {t("driver.tabs.myTrip.publishedRides.activeSuffix")}
        </Text>
      </View>
