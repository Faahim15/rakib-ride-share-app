...
import FormSkeleton from "@/src/components/skeleton/rakib-form-skeleton";
import { useLanguage } from "@/src/localization/LangaugeContext";
import { useGetAllCountriesQuery } from "@/src/redux/country-slice/coutries-api";
...
export default function RakibPaymentForm() {
  ...
  const [unlimitedMode, setUnlimitedMode] = useState("basic");
  const [unlimitedModeLabel, setUnlimitedModeLabel] = useState(
    t("shared.rakibPaymentForm.selectMode"),
  );
  const { id } = useLocalSearchParams();
  const [fullName, setFullName] = useState("Letizia");
  const [email, setEmail] = useState("letizia@example.com");
  const [rakibUsername, setRakibUsername] = useState("letizia");
  ...
  const plan = normalizedId === "getplus" || normalizedId === "allAccess";
            ...
            {id === "premium" && (
              <View className="flex-row flex-wrap">
                ...
                <TouchableOpacity
                  onPress={() =>
                    router.push({
                      pathname: "/shared/upgrade-to-allAccess",
                      params: { id: "allAccess" },
                    })
                  }
                >
